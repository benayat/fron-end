...
function randomCard(array) {
  for (let i = 0; i < 12; i++) {
    document
      ...
      [i].classList.add(`background-${array[i]}`);
  }
}
    ...
    if (runing === true) {
      ...
      if (seconds === run.getSeconds()) {
        document.querySelector(
          `.heading`
        ).innerHTML = `<h1 style="color:red">Time Out!<br> Game over!</h1>`;
      }
    }
  }, 10);
}
...
function resetGame() {
  document.querySelector(
    `.heading`
  ).innerHTML = `<h1>Memory Game! <small><br>By team Seven</small> </h1>`;
  init();
}
...
function isWon() {
  if (state.numberOfRightGuesses === state.numberOfCouples) {
    document.querySelector(
      `.heading`
    ).innerHTML = `<h1>Winner!<br> Amazing!</h1><h3>Watch your stats and improve next time</h3>`;
    let winTime = document.querySelector(`.timer`).textContent;
    stopAndRestart();
    document.querySelector(
      `.timer`
    ).innerHTML = `<p style="color:green">${winTime}</p>`;
  }
}
